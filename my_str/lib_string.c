...
int my_str_reserve(my_str_t* str, size_t buf_size){
    if (buf_size >= str->size_m){
        char* new_data = malloc(buf_size + 1);
        if (new_data != NULL){
            for (size_t i = 0; i < str->size_m; i++){
                new_data[i] = my_str_getc(str, i);
            }
...
int my_str_pushback(my_str_t* str, char c){
    if (str->size_m == str->capacity_m){
        my_str_reserve(str, reserve_coef * str->capacity_m);
    }
...
int my_str_copy(const my_str_t* from,  my_str_t* to, int reserve){
    ...
    while (from->size_m > to->capacity_m){
        my_str_reserve(to, reserve_coef * to->capacity_m);
    }
    my_str_clear(to);
    char c;
...
int my_str_insert(my_str_t* str, const my_str_t* from, size_t pos){
    while (str->capacity_m < str->size_m + from->size_m){
        my_str_reserve(str, reserve_coef * str->capacity_m);
    }
...
int my_str_insert_cstr(my_str_t* str, const char* from, size_t pos){
    ...
    while (str->capacity_m < str->size_m + len_from){
        my_str_reserve(str, reserve_coef*str->capacity_m);
    }
...
int my_str_append_cstr(my_str_t* str, const char* from){
    while (str->capacity_m < str->size_m + len_cstr(from)){
        my_str_reserve(str, reserve_coef*str->capacity_m);
    }
